...
"""
    Mon CA a three states, for now it's a copy of Baricelli 3D.
...
class MonCA(Automaton):
    """
        Baricelli model in 2D. 
    """

    def __init__(self, size, n_species : int=3, reprod_collision=False, device='cpu'):
        """
            Params:
            size : 2-uple (H,W)
    ...
    def step(self):
        ...
        new_tensor = self.world.detach().clone() #je fais une deep copy 
        new_tensor[:,:,1] = 0 # je vais regarder que la dimension 1. 
        
        new_world = ((self.world + new_tensor)%self.t_size[None,None,:])[nonzeromask]
        #make the move : 
        self.time+=1
        ...
        nonzeromask = self.world!=0 # (H,W,2), mask of nonzero cells
        nonzeromask = nonzeromask.any(dim=-1) # (H,W), mask of moving cells
        # le dim = -1 renvoie a la derniere dimension. Donc la ca garde que ceux qui ont qqchose
        # dans la derniere dim. 
        
        ## First, move
        tar_pos = ((self.positions+self.world)%self.t_size[None,None,:])[nonzeromask] # (N,2), target positions of nonzero cells
        # tar_pos c'est un array de 117611 *2 (i.e. (N,2)) 
        #self.positions c'est un tensor array de h*w*2. (les pos x,y)
        #self.world c'est un tensor array de h*w*2. (dans la dim 1 on a la valeur de l'espece qui fait bouger sur x,
        # et dans la dim 2 celle qui fait bouger sur y. moi il me faudra plus de dim :
        # first dim : (0,1,2) (no hamster, susceptible, infected), death time etc
